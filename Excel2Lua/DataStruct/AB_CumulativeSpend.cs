...
namespace Excel2Lua
{
	public class AB_CumulativeSpend : ActivityBase
	{
		...
		{
			bool bReturn = true;
			foreach (CumulativeSpendInfo info in m_listOwn)
			{
				bool bCheckMale = ItemMgr.Instance.CheckItemAndLogError(m_strExcelSheet, (uint)info.malereward_itemid, info.malereward_itemcount, info.malereward_itemvalidity);
				bool bCheckFemale = ItemMgr.Instance.CheckItemAndLogError(m_strExcelSheet, (uint)info.femalereward_itemid, info.femalereward_itemcount, info.femalereward_itemvalidity);
